...
if (eeg_active):
    ...
    # (train data is loaded to compute the normalization parameters only)
...
if (not(eog_active) and not(emg_active)):
    ...
    test_generator.data_shape_2 = test_generator.X.shape[1:]
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        # read optimal weight
        with open(os.path.join(out_dir, "final_w.txt"), "r") as w_file:
        ...
        def evaluate(gen):
            # Validate the model on the entire data
            output_loss1 =0
            output_loss2 =0
            output_loss3 =0
            output_loss =0
            ...
            # test with 10x of batch size to speed up
            ...
            while test_step < num_batch_per_epoch:
                ...
                for n in range(config.epoch_seq_len):
                    ...
                    deep_score[n, (test_step-1)*10*config.batch_size : test_step*10*config.batch_size,:] = deep_score_[n]
                    seq_score[n, (test_step-1)*10*config.batch_size : test_step*10*config.batch_size,:] = seq_score_[n]
                    joint_score[n, (test_step-1)*10*config.batch_size : test_step*10*config.batch_size,:] = joint_score_[n]
            ...
            if(gen.pointer < len(gen.data_index)):
                ...
                for n in range(config.epoch_seq_len):
                    ...
                    deep_score[n, (test_step-1)*10*config.batch_size : len(gen.data_index),:] = deep_score_[n]
                    seq_score[n, (test_step-1)*10*config.batch_size : len(gen.data_index),:] = seq_score_[n]
                    joint_score[n, (test_step-1)*10*config.batch_size : len(gen.data_index),:] = joint_score_[n]
        ...
        savemat(os.path.join(out_path, "test_ret.mat"), dict(yhat = test_yhat, acc = test_acc, score = test_score, output_loss = test_loss,
